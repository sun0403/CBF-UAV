...
class NloptControl(BaseControl):
    ...
    def __init__(self,
        ...
        self.gp_model = GaussianProcessCBF(input_dim=6)
    ...
    def _dslPIDPositionControl(self,
        ...
        u_opt = self._optimize_control(target_thrust, cur_pos, cur_vel, cur_quat)
    ...
    def _initialize_gp_samples(self, num_samples=10, pos_center=np.array([0, 0, 0.5]), vel_range=0.5):
        ...
        for i in range(num_samples):
            ...
            cbf_val = -(h_ddot + self.lambda1 * h_dot + self.lambda2 * h)
...
#         print(f"  target_thrust = {target_thrust}")
#         scalar_thrust = max(0., np.dot(u_opt, cur_rotation[:,2]))
#         thrust = (math.sqrt(scalar_thrust / (4*self.KF)) - self.PWM2RPM_CONST) / self.PWM2RPM_SCALE
